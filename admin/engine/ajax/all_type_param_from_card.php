        ...
        foreach ($array_params_for_card as $key => $v)
        {
            ...
            switch ($v['db_type'])
            {
                case "text":
                    ...
                    $option .= '<option value="'.$v['id_param'].'|text">'.$param_translate['text'].' | Тип: Текст</option>';
                break;
                case "select":
                    $param_translate = return_name_for_id_select_param ($id_online_lang, $v['id_param']);
                    $option .= '<option value="'.$v['id_param'].'|select">'.$param_translate['text'].' | Тип: Выбор</option>';
                break;
                case "price":
                    $info_price_param_translate = return_name_for_id_price_param ($id_online_lang, $v['id_param']);
                    $option .= '<option value="'.$v['id_param'].'|price">'.$info_price_param_translate['text'].' | Тип: Цена</option>';
                break;
            }
        }
