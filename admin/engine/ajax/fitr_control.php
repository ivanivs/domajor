...
if (check_user())
{
    ...
    <input type="hidden" name="id_filtr" value="'.$_POST['id'].'">
    ...
    if ($array_all_select_params)
    {
        foreach ($array_all_select_params as $v_param)
        {
            ...
            foreach ($all_values_of_select_param as $v_value)
            {
                ...
                if (check_filtr_param($_POST['id'], $v_param['id'], $v_value['id']))
                {
                    $checked = 'checked';
                } else {
                    $checked = '';
                }
                print '<p style="padding-left:30px;"><input type="checkbox" name="filtr_param[]" value="'.$v_param['id'].'_'.$v_value['id'].'" '.$checked.'>'.$parent_name.$info_select_value_tr['text'].'</p>';
