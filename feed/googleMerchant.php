...
$body = '<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
<channel>
<title>Kombat.in.ua</title>
<link>https://kombat.in.ua</link>
<currencies>
...
$body .= '</categories>';
//$body .= '<items>';
if ($arrayItems = getArray("SELECT * FROM `ls_items` WHERE `select_5` = 76;")){
    foreach ($arrayItems as $v){
        ...
        if ($infoItem['price_2']==0 or $infoItem['price_1']==$infoItem['price_2'])
        {
            ...
            $priceHtml = $infoItem['price_1'];
            $salePrice = '<g:sale_price>'.$infoItem['price_2'].' UAH</g:sale_price>';
        }
        ...
        $pictures = Array();
        $mainImage = getMainImage ($infoItem['id']);
        $additional_image_link = '';
        foreach ($infoAllPhoto as $key => $oneP)
        {
            $pictures .= $config['site_url'] . 'upload/userparams/' . $oneP['value'];
            if ($oneP['value']!=$mainImage and $key<10)
                $additional_image_link .= '<g:additional_image_link><![CDATA['.$config ['site_url'].'resize_image.php?filename='.urlencode( $oneP['value']).'&const=129&r=255&g=255&b=255&width=480&height=400'.$idCard.']]></g:additional_image_link>'."\r\n";
        }
        //
        ...
        $body .= '
                <item id="'.$v['id'].'" available="true" selling_type="r">
                    ...
                    <title>'.htmlspecialchars(getNameItem($v)).'</title>
                    <link>https://kombat.in.ua/ua/mode/item-'.$infoItem['id'].'.html</link>
                    <g:availability>in stock</g:availability>
                    <g:categoryId>'.$infoItem['select_1'].'</g:categoryId>
                    <g:mpn>'.$infoItem['text_2'].'</g:mpn>
                    <g:price>'.$priceHtml.' UAH</g:price>
                    '.$salePrice.'
                    <g:brand>'.htmlspecialchars(getOneValueText($infoItem['select_4' .
            ''])).'</g:brand>
                    <g:image_link>
                        <![CDATA['.$config ['site_url'].'resize_image.php?filename='.urlencode($mainImage).'&const=129&r=255&g=255&b=255&width=480&height=400'.$idCard.']]>
                    </g:image_link>
                    '.$additional_image_link.'
                    <description><![CDATA['.((!empty($infoItem['text_7'])) ? $infoItem['text_7'] : $infoItem['text_3']).']]></description>
                </item>
                ';
        /*
